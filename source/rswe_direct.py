...
def dissipative_exponential(control, expInt, U_hat, t):
    ...
    **Returns**
    ...
    kc = 21  # Normalisation regardless of grid-scale
    ...
    p = 8  # 4-th order is nabla^8, a.k.a. delta^4
...
def compute_average_force(U_hat, control, st, expInt):
    ...
    *See Also**
    ...
    for m in np.arange(1,M):  # NB This loop is embarassingly parallelisable (on its own)
        tm = T0*m/float(M)
...
def solve(solver_name, control, st, expInt, u_init, invert_fft = True):
    ...
    **Notes**
    ...
    if invert_fft:  # i.e. initial condition is in realspace
